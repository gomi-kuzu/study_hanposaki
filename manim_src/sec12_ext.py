...
class LaplaceTransform(Scene):
    def construct(self):
        ...
        # タイトル
        title = Text("ラプラス変換（おまけ）", font_size=34, color=WHITE)
        title.to_edge(UP)
        self.play(Write(title), run_time=0.8)
        self.wait(0.8)

        # === Part 1: 導入 ===
        subtitle1 = Text("フーリエ変換に似た変換がある", font_size=30, color=BLUE)
        ...
        intro_note = VGroup(
            ...
            Text("それが「ラプラス変換」", color=YELLOW, font_size=28, weight=BOLD),
        ).arrange(DOWN, buff=0.2)
        ...
        s_note = VGroup(
            Text("ただし", color=WHITE, font_size=24),
            MathTex(r"s = \sigma + j\omega", color=ORANGE, font_size=28),
            Text("（複素数）", color=WHITE, font_size=24),
        ...
        # 違いを強調
        diff_group = VGroup(
            Text("違い①: 積分区間が 0 → ∞（因果信号を想定）", color=RED, font_size=21),
            VGroup(
                Text("違い②: e の肩が ", color=RED, font_size=21),
                MathTex(r"-j\omega t", color=TEAL, font_size=22),
                Text(" →  ", color=WHITE, font_size=21),
                MathTex(r"-st = -(\sigma + j\omega)t", color=YELLOW, font_size=22),
        ...
        basis_intro = Text("ラプラス変換の核（カーネル）を見てみよう:", color=WHITE, font_size=24)
        ...
        # 2つの成分に分解して説明
        comp1 = VGroup(
            MathTex(r"e^{-j\omega t}", color=TEAL, font_size=30),
            Text(": 振動成分", color=TEAL, font_size=24, weight=BOLD),
        ...
        comp1_detail = Text("→ どのくらい強い振動成分があるかがわかる", color=TEAL, font_size=22)
        comp1_detail.shift(DOWN * 0.4)
        self.play(Write(comp1_detail), run_time=0.5)
        self.wait(0.3)

        comp2 = VGroup(
            MathTex(r"e^{-\sigma t}", color=ORANGE, font_size=30),
            Text(": 減衰・発散成分", color=ORANGE, font_size=24, weight=BOLD),
        ...
        comp2_detail = Text("→ 信号が収束するのか発散するのかがわかる", color=ORANGE, font_size=22)
        ...
        decay_label = VGroup(
            MathTex(r"\sigma > 0", color=BLUE, font_size=24),
            Text(": 減衰（収束）", color=BLUE, font_size=20),
        ).arrange(RIGHT, buff=0.1)
        ...
        flat_label = VGroup(
            MathTex(r"\sigma = 0", color=GREEN, font_size=24),
            Text(": 定常（フーリエと同じ）", color=GREEN, font_size=20),
        ).arrange(RIGHT, buff=0.1)
        ...
        grow_label = VGroup(
            MathTex(r"\sigma < 0", color=RED, font_size=24),
            Text(": 発散", color=RED, font_size=20),
        ).arrange(RIGHT, buff=0.1)
        ...
        combo_intro = Text("実際のラプラス基底は振動成分と減衰成分の積:", color=WHITE, font_size=22)
        ...
        osc_label = VGroup(
            MathTex(r"e^{-j\omega t}", color=TEAL, font_size=22),
            Text(": 振動のみ（σ=0）", color=TEAL, font_size=20),
        ).arrange(RIGHT, buff=0.1)
        osc_label.next_to(combo_axes, UP, buff=0.1).shift(LEFT * 1)
        ...
        decay_osc_label = VGroup(
            MathTex(r"e^{-\sigma t} \cdot e^{-j\omega t}", color=ORANGE, font_size=22),
            Text(": 振動しながら減衰（σ>0）", color=ORANGE, font_size=20),
        ).arrange(RIGHT, buff=0.1)
        decay_osc_label.next_to(combo_axes, DOWN, buff=0.1).shift(LEFT * 0.5)
        ...
        combo_summary = VGroup(
            Text("ラプラス変換は", color=WHITE, font_size=22),
            Text("「振動の速さ（周波数）」", color=TEAL, font_size=22, weight=BOLD),
            Text("と", color=WHITE, font_size=22),
            Text("「収束・発散の速さ」", color=ORANGE, font_size=22, weight=BOLD),
            Text("を同時に解析！", color=YELLOW, font_size=22, weight=BOLD),
        ).arrange(RIGHT, buff=0.1)
        ...
        ctrl_intro = VGroup(
            Text("制御工学では入出力の関係を", color=WHITE, font_size=24),
